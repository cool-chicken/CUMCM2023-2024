...
$M'$为新轮次中成品次品数目。则零配件池中次品的平配件数$n_{1}$、$n_{2}$为：
\begin{equation}
...
\begin{figure}[H]
	...
	\label{fig:pro2-2}
\end{figure}
综上所述，我们建立了一个基于生产周期的决策模型。通过优化每个生产周期内的决策向量 $C^{*}$，我们能够获得使预期净利润最大化的最优决策方案：
...
\begin{adjustbox}{width=16cm,height=6.5cm}
	...
	\end{algorithm}
\end{adjustbox}
首先，我们需要设定合理的变异率、交叉率、生产周期长度 t 以及各项成本参数。随后，我们随机生成初始决策向量种群，
并计算每个个体的适应度。基于适应度，我们选择优良个体进行交叉和变异操作，生成新的子代种群。最后，我们从父代和子代
种群中选择适应度最高的个体组成新的父代种群，并重复上述过程，直至找到最优决策向量 $\vec{C}$ 。
...
\begin{thebibliography}{9}
	\bibitem[1]{SPRT}Optimum character of the sequential probability ratio test, A Wald, J Wolfowitz - The Annals of Mathematical Statistics, 1948 - JSTOR
